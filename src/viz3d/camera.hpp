...
namespace viz {
    ...
    public:
        ...
        glMatrix4f WorldToCam() const {
            ...
            pose.block<3, 3>(0, 0) = orientation_.transpose();
            pose.block<3, 1>(0, 3) = -orientation_.transpose() * location_;
        ...
        Eigen::Vector3d FrontVector() const {
            return orientation_.block<3, 1>(0, 2);
        }

        Eigen::Vector3d RightVector() const {
            return orientation_.block<3, 1>(0, 0);
        }

        Eigen::Vector3d UpVector() const {
            return orientation_.block<3, 1>(0, 1);
        }
    ...
    protected:
        ...
        Eigen::Vector3d location_ = Eigen::Vector3d(0, 0, 10);
